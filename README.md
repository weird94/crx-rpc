...
## Serving RPC from Content Scripts (Background → Tab)

### 1. Register a service inside the content script
...
host.register(IPageInfoService, {
    async ping(name: string) {
        // This logic runs in the tab (content script / web page)
        return `pong from tab: ${name}`;
    },
});

// Optionally forward calls to the web page by also creating a ContentRPC bridge
// const bridge = new ContentRPC();
```

The `ContentRPCHost` mirrors `BackgroundRPC` but listens for calls coming from the background.
Any service registered on the host can be invoked from the background side.

### 2. Call the tab service from the background script
...
async function callContentService(tabId: number) {
    ...
    client.dispose(); // Clean up listeners when the call flow is done
    return result;
}
```

`TabRPCClient` internally uses a `TabMessageAdapter` powered by
[`@webext-core/messaging`](./packages/webext-core-messaging/index.js) to target the specified tab in a
browser-agnostic way. The adapter automatically selects the proper `browser.*` or `chrome.*`
runtime APIs, so the same RPC client works across Chromium- and Firefox-based browsers. The
client behaves like any other `RPCClient`, so you can reuse the same interface identifiers to
call content-script services.

> **Note:** Both `ContentRPCHost` and `ContentRPC` use the same messaging wrapper, ensuring that
> background ↔ content communication keeps working even when the extension is executed in browsers
> that only expose the Promise-based `browser.runtime` APIs.
